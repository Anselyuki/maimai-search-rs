...
impl MaimaiDB {
    ...
    pub fn search_song_list(sql: &str) -> Vec<Song> {
        let connection = MaimaiDB::get_connection();
        let mut statement = connection.prepare(sql).unwrap();
        statement.query_map(params![], parse_row).unwrap()
...
fn parse_row(row: &Row) -> Result<Song, rusqlite::Error> {
    let song = Song {
        ...
        ds: serde_json::from_str(row.get::<usize, String>(3).unwrap().as_str()).expect("谱面定数序列化错误"),
        level: serde_json::from_str(row.get::<usize, String>(4).unwrap().as_str()).expect("谱面等级序列化错误"),
        cids: serde_json::from_str(row.get::<usize, String>(5).unwrap().as_str()).expect("谱面ID序列化错误"),
        charts: serde_json::from_str::<Vec<Chart>>(row.get::<usize, String>(6).unwrap().as_str()).expect("Chart 序列化错误"),
        basic_info: serde_json::from_str::<BasicInfo>(row.get::<usize, String>(7).unwrap().as_str()).expect("basic_info 序列化错误"),
    };
    Ok(song)
}